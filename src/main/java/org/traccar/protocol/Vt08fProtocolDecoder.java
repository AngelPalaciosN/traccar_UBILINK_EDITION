...
public class Vt08fProtocolDecoder extends BaseProtocolDecoder {

    public Vt08fProtocolDecoder(Protocol protocol) {
        super(protocol);
    }
    ...
    private boolean decodeLbs(Position position, ByteBuf buf, int type, boolean hasLength) {
        ...
        // VT08F Specific LBS Block
        int mcc = buf.readUnsignedShort();
        int mnc = buf.readUnsignedShort(); // VT08F uses 2 bytes for MNC
        int lac = buf.readUnsignedShort(); // VT08F uses 2 bytes for LAC
        long cid = buf.readUnsignedInt();  // VT08F uses 4 bytes for Cell ID

        position.setNetwork(new Network(CellTower.from(mcc, mnc, lac, cid)));

        if (length > 9) {
            buf.skipBytes(length - 9); // length usually includes some other data, but GT06 just skips remaining
        }
    ...
    private Object decodeBasic(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        if (type == MSG_LOGIN) {
            ...
            if (type == MSG_GPS_LBS_1) {
                if (variant == Variant.Vt08fE_CARD) {
                    position.set(Position.KEY_ODOMETER, buf.readUnsignedInt());
